...
JSON_LOCATION="/docker/.docker/config.json"
...
while true; do
    ...
    if [ -z "$S3_PATH" ]; then
        ...
        aws s3 cp /docker/docker.tar.gz s3://${S3_BUCKET}/docker.tar.gz
    else
        log "Uploading to s3://$S3_BUCKET/$S3_PATH/docker.tar.gz"
        aws s3 cp /docker/docker.tar.gz s3://${S3_BUCKET}${S3_PATH}/docker.tar.gz
    fi

    log "Sleeping for $INTERVAL"
    sleep ${INTERVAL}

    log "Removing /docker/docker.tar.gz"
    rm -f /docker/docker.tar.gz
done